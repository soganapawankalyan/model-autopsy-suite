...
def run_shap_analysis(model, X, sample_idx):
    ...
    try:
        ...
        return {"contribs": [], "base_value": 0, "prediction_value": 0, "valid": False, "error": str(e)}
...
def run_confidence_decomposition(model, X, sample_idx):
    ...
    if hasattr(model, "estimators_"):
        tree_preds = np.array([est.predict_proba(sample)[0][pred_class] for est in model.estimators_])
        tree_std = float(np.std(tree_preds))
        tree_mean = float(np.mean(tree_preds))
        disagreement = tree_std
    else:
        tree_std = tree_mean = disagreement = 0.0
...
def classify_failure_severity(model, X, y, sample_idx, nn_result, conf_result):
    ...
    if is_wrong and conf > 0.75 and iso > 0.4:
        ...
        severity = "marginal"
        score = 0.35
        reason = "Low-confidence wrong prediction near the decision boundary — model is uncertain but tips the wrong way"
    else:
        severity = "correct"
        score = 0.0
...
if __name__ == "__main__":
    ...
    for name, fn in ALL_CASES.items():
        ...
        if result["shap"]["valid"]:
            ...
            for c in top3:
                direction = "up" if c["shap"] > 0 else "down"
                print(f"    {direction} {c['feature']}: {c['value']} (SHAP={c['shap']:+.4f})")
'''
